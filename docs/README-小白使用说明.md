...
> 当前源码版：1.6.2；适用电脑：Windows 10 / Windows 11 64 位；适用 VPS：Ubuntu 或
...
如果希望继续使用原来的 Tkinter 界面，可以运行体积更小的兼容版本：

```text
TG115-Deployer-Classic.exe
```

Modern 是默认推荐版本；Classic 保留原界面，不承诺同步后续界面功能。启动
`一键部署-Telegram到115.cmd` 时会优先打开 Modern，找不到时再回退到 Classic。
