...
> 公开正式版：1.6.1；适用电脑：Windows 10 / Windows 11 64 位；适用 VPS：Ubuntu 或
...
点击按钮会更新当前这一条菜单消息，不会连续发送很多新消息。按钮只能由配置的本人账号在与
Bot 的私聊中使用；文字命令仍然全部有效。

最近任务每页显示 5 项，使用“上一页”“下一页”翻页，也可以点击任务编号进入详情。详情页会
根据任务状态显示“刷新进度”“重新排队”“切换流式”和“取消任务”。点击“取消任务”只是进入
确认页，必须在 5 分钟内再次点击“确认取消”才会开始安全清理；点击“放弃操作”、按钮过期或
远端无法确认删除时都不会冒险删除本地数据。直接输入 `/cancel <任务编号>` 的旧用法继续有效。
快捷“临时巡检”只做只读检查；删除遗留文件仍需输入 `/orphans clean` 并按一次性确认码操作。
...
新版测试和检查结果见 [v1.6.1 更新与验收记录](v1.6.1-更新与验收.md)。
