...
先点击“测试 SSH”。连接成功后，部署器会同时读取 VPS 资源，并在“可选设置”页显示当前
实例的“均衡模式”和“流式优先”建议。
...
上面仍是源码默认值，不会因为检测而自动变化。你可以在“可选设置”页点击：
...
13. 如果需要保留人工核验记录，可发送 `/confirm <任务编号>`；文件较多时，集中核验后发送
    一次 `/confirm all`，最多批量确认 100 个，仍有更多时可再次执行。

人工确认记录是可选的。`/confirm` 只记录你的人工确认，不会重新上传，也不会修改或删除 115 中的文件。Bot
...
### 115 官方客户端已经能看到文件，还需要逐个确认吗

不需要。`Bot 传输已完成（CloudDrive2 已接收）` 表示 VPS 已经把文件写入
CloudDrive2 WebDAV，并完成远端大小校验和本地清理；它不是“仍在占用 VPS 上传带宽”。

确认 115 官方客户端中的文件大小正常并可以打开或播放后，如需保留人工记录，可以发送：

```text
/confirm <任务编号>
```

批量文件集中核验后，发送 `/confirm all` 即可一次确认最近最多 100 个，不需要逐条操作。
...
新版测试和检查结果见 [v1.6.0 优化与验收记录](v1.6.0-优化与验收.md)。
