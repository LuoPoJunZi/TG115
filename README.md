...
- `completed` 表示 Bot 传输完成；只有你在 115 官方客户端核验并执行 `/confirm` 后，才记录为
  `confirmed`。
- `/confirm` 是可选的人工记录，不会再次上传、移动或删除文件；文件多时可以使用
  `/confirm all`。
...
再填写计划在 CloudDrive2 中创建的 WebDAV 用户名、密码和可选相对子目录。不要填写 VPS 公网
地址；Bot 与 CloudDrive2 通过 Docker 内网通信。
...
该地址只通过当前 SSH 隧道访问，CloudDrive2 的 19798 端口默认不会暴露到公网。
...
9. 在 115 官方客户端检查文件；
10. 如需留档，执行 `/confirm <编号>` 或 `/confirm all`。
...
| `/start`、`/help` | 查看帮助 |
| `/queue` | 查看队列摘要 |
| `/status` | 查看资源、目的端、实际传输数和任务统计 |
| `/performance` | 查看阶段速度和动态窗口 |
...
| `/confirm <编号>` | 记录一个任务已由用户在 115 官方端核验 |
| `/confirm all` | 批量确认最近最多 100 个已完成任务 |
...
不需要。人工确认只是可选记录。集中在 115 官方客户端核验后执行 `/confirm all`，即可一次
确认最近最多 100 个 `completed` 任务；仍有更多时可以再次执行。
